...
class Camera:
    def __init__(self, hubs: dict, screen_width: int, screen_height: int) -> None:
        ...
        self.offset_x = (self.screen_width / 2) - (center_logical_x * self.scale)
        self.offset_y = (self.screen_height / 2) + (center_logical_y * self.scale)

    def get_screen_coords(self, logical_x: float, logical_y: float) -> tuple[int, int]:
        """Convertit les positions logiques en pixels écran."""
        screen_x = int(logical_x * self.scale + self.offset_x + self.add_x)
        screen_y = int(-logical_y * self.scale + self.offset_y + self.add_y)
        return screen_x, screen_y

    def update_screen_coords(self, add_x: int, add_y: int):
        self.add_x += add_x
        self.add_y += add_y

    def zoom(self, scroll: int):
        self.scale += scroll
