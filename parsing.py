    ...
    def valid_start_end(self):
        ...
        for hub in self.hubs:
            if hub.is_start and is_start:
                raise ValidationError("Input Error: It can only have one start hub")
            if hub.is_end and is_end:
                raise ValidationError("Input Error: It can only have one end hub")
...
def create_hub_metadata(datas: str, hub_data: Dict[str, Any]) -> tuple[str, Dict[str, Any]]:
...
def create_hub(line: str) -> Hub:
    ...
    hub_data["name"] = lst_major[0]
    hub_data["x"] = int(lst_major[1])
    hub_data["y"] = int(lst_major[2])
    return Hub(**hub_data)


def create_connection(line: str, lst_hubs: List[Hub], connections: List[Connection]) -> Connection:
    try:
        title, datas = line.split(": ")
    except Exception:
    ...
    if [hub1, hub2] in lst_connection or [hub2, hub1] in lst_connection:
        raise ValueError(f"Input Error: {datas}, this connection already exist")
    return Connection(hub1=hub1, hub2=hub2, max_link_capacity=max_link)


def drone_line(line: str, idx: int, nb_drone: int) -> int:
    if nb_drone != None:
        raise ValueError(f"Error in line {idx}: Input Error: nb_drones is already set")
    try:
        nb_drone = int(line.split(":")[1])
    except Exception:
        raise ValueError(f"Error in line {idx}: Input Error: nb_drones must be an integer")
    if nb_drone < 0:
        raise ValueError(f"Error in line {idx}: Input Error: nb_drones must be positive")
    return nb_drone
...
def parse_file(filename: str):
    ...
    nb_drone: int|None = None
    hubs: List[Hub] = []
    connections: List[Connection] = []
    for idx, line in enumerate(lst):
        if line.startswith("nb_drones: "):
            try:
                nb_drone = drone_line(line, idx+1, nb_drone)
            except Exception as e:
                raise ValueError(e)
        elif line.startswith(("hub:", "start_hub:", "end_hub:")):
            if nb_drone == None:
                raise ValueError(f"Error in line {idx+1}: Input Error: The file must start with nb_drones: X")
            try:
...
try:
    input_datas: Input_Datas = parse_file("maps/easy/03_basic_capacity.txt")
